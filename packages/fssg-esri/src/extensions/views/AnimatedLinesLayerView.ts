...
// @ts-ignore
export const AnimatedLinesLayerView = BaseLayerViewGL2D.createSubclass({
  ...
  attach: function () {
    ...
    const trailSpeed = ((animatedOptions?.trailSpeed ?? 50.0) as number).toFixed(2)
    const trailLength = ((animatedOptions?.trailLength ?? 300.0) as number).toFixed(2)
    const trailCycle = ((animatedOptions?.trailCycle ?? 1000.0) as number).toFixed(2)
  ...
  updatePositions: function (renderParameters) {
    ...
    const animatedOptions = this.layer.animatedOptions
    const defaultColor = animatedOptions?.defaultColor ?? [255, 0, 0]
    ...
    for (let i = 0; i < graphics.items.length; ++i) {
      const graphic = graphics.items[i]
      const path = graphic.geometry.paths[0]
      const color = graphic.attributes?.['color'] ?? defaultColor

      // Initialize new triangulation state.
      const s = {} as any // eslint-disable-line
...
export default AnimatedLinesLayerView
